...
# let's now split up the data into train and valid sets
data = torch.tensor(encode(text), dtype=torch.long)
n = int(0.9*len(data)) # first 90% goes to the train set, rest goes to valid set
train_data = data[:n]
valid_data = data[n:]
...
class BigramLanguageModel(nn.Module):
  ...
  def generate(self, idx, max_new_tokens):
    ...
    for _ in range(max_new_tokens):
      # crop idx to the last size token
...
print("parameters = ", sum(p.numel() for p in model.parameters())) # 10.788.929
